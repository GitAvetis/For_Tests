...
/// @brief Главное окно приложения для работы с лабиринтами
using System;
...
{
    /// <summary>
    /// @brief Главное окно приложения для генерации, редактирования и решения лабиринтов
    /// 
    /// Класс MainWindow предоставляет графический интерфейс для работы с лабиринтами.
    /// Позволяет генерировать новые лабиринты, загружать их из файлов, сохранять,
    /// задавать начальную и конечную точки, а также находить путь между ними.
    /// </summary>
    public partial class MainWindow : Window
    {
        /// @name Константы отображения
        /// @{
        private const int MazeImageSize = 500;           ///< Размер изображения лабиринта в пикселях
        private const int WallThickness = 2;              ///< Толщина стен лабиринта
        private const int SolutionLineThickness = 2;      ///< Толщина линии решения
        private const int BitmapDpi = 96;                  ///< DPI для создания растрового изображения
        /// @}

        private Core.Maze _currentMaze;                    ///< Текущий лабиринт
        private readonly MazeFileService _mazeFileService; ///< Сервис для работы с файлами лабиринтов
        private readonly Core.MazeSolver _mazeSolver;      ///< Сервис для решения лабиринтов

        private Core.Point? _startPoint;                    ///< Точка старта
        private Core.Point? _endPoint;                      ///< Точка финиша
        private List<Core.Point> _solutionPath;             ///< Найденный путь решения
        private bool _isSelectingStartPoint;                 ///< Флаг режима выбора стартовой точки
        private bool _isSelectingEndPoint;                   ///< Флаг режима выбора конечной точки

        /// <summary>
        /// @brief Конструктор главного окна
        /// 
        /// Инициализирует компоненты пользовательского интерфейса,
        /// создает необходимые сервисы и генерирует начальный лабиринт.
        /// </summary>
        public MainWindow()
        {
            InitializeComponent();
        ...
        /// <summary>
        /// @brief Обработчик нажатия кнопки генерации нового лабиринта
        /// </summary>
        ...
        /// <summary>
        /// @brief Генерирует новый лабиринт с размерами из слайдеров
        /// </summary>
        ...
        /// <summary>
        /// @brief Отрисовывает текущий лабиринт на холсте
        /// 
        /// Метод создает визуальное представление лабиринта с учетом:
        /// - стен лабиринта
        /// - найденного пути решения (красным цветом)
        /// - стартовой точки (зеленым кругом)
        /// - конечной точки (синим кругом)
        /// 
        /// Использует DrawingVisual для отрисовки и RenderTargetBitmap для создания растрового изображения.
        /// </summary>
        ...
        /// <summary>
        /// @brief Обработчик нажатия кнопки загрузки лабиринта из файла
        /// </summary>
        /// <param name="sender">Источник события</param>
        /// <param name="e">Аргументы события</param>
        ...
        /// <summary>
        /// @brief Обработчик нажатия кнопки сохранения лабиринта в файл
        /// </summary>
        /// <param name="sender">Источник события</param>
        /// <param name="e">Аргументы события</param>
        ...
        /// <summary>
        /// @brief Обработчик нажатия кнопки решения лабиринта
        /// 
        /// Запускает поиск пути от стартовой до конечной точки.
        /// Для работы необходимо предварительно задать обе точки.
        /// </summary>
        /// <param name="sender">Источник события</param>
        /// <param name="e">Аргументы события</param>
        ...
        /// <summary>
        /// @brief Обработчик нажатия кнопки выбора стартовой точки
        /// </summary>
        /// <param name="sender">Источник события</param>
        /// <param name="e">Аргументы события</param>
        ...
        /// <summary>
        /// @brief Обработчик нажатия кнопки выбора конечной точки
        /// </summary>
        /// <param name="sender">Источник события</param>
        /// <param name="e">Аргументы события</param>
        ...
        /// <summary>
        /// @brief Обработчик клика левой кнопкой мыши по изображению лабиринта
        /// 
        /// Преобразует координаты клика в координаты ячейки лабиринта
        /// и в зависимости от текущего режима устанавливает стартовую или конечную точку.
        /// </summary>
        /// <param name="sender">Источник события</param>
